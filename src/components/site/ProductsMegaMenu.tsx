...
export function ProductsMegaMenu({ open, categories, onNavigate, className }: Props) {
  ...
  return (
    <div
      ...
      className={cn(
        "mega-panel absolute inset-x-3 top-2 z-50 overflow-hidden rounded-2xl border border-white/10 bg-ink-soft shadow-2xl shadow-black/40 sm:inset-x-6 lg:inset-x-8",
        className,
      )}
      role="menu"
      aria-hidden={!open}
    >
      <div className="max-h-[min(70vh,520px)] overflow-y-auto overscroll-contain p-4 sm:p-5">
        {cols.length === 0 ? (
          <p className="px-2 py-6 text-center text-sm text-white/50">هنوز دسته‌بندی‌ای ثبت نشده است.</p>
        ) : (
            ...
            {cols.map((cat, i) => (
              ...
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <Link
                      ...
                      onClick={onNavigate}
                      className="block text-[15px] font-bold leading-7 text-white transition hover:text-copper"
                    >
                      {cat.nameFa}
                ...
                <ul className="space-y-1.5">
                  {(cat.children?.length ? cat.children : [{ slug: cat.slug, nameFa: "همه موارد این دسته", productCount: cat.productCount }]).map(
                    (child) => (
                      <li key={child.slug}>
                        <Link
                          href={`/products?category=${child.slug}`}
                          onClick={onNavigate}
                          className="group flex items-center gap-2.5 rounded-lg px-1.5 py-1.5 text-sm text-white/70 transition hover:bg-white/5 hover:text-white"
                        >
                          ...
                          {typeof child.productCount === "number" && child.productCount > 0 ? (
                            <span className="tabular-nums text-[11px] text-white/35 group-hover:text-copper">
